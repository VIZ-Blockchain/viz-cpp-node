...
tar xjf boost_1_8_0.tar.bz2
cd boost_1_84_0
./bootstrap.sh --with-toolset=gcc
./b2 -j4     toolset=gcc     address-model=64     variant=release     link=static     threading=multi     runtime-link=static     context-impl=fcontext     define=_WIN32_WINNT=0x0601     define=WINVER=0x0601     cxxflags="-march=x86-64 -mtune=generic"     cflags="-march=x86-64 -mtune=generic"     --with-atomic --with-chrono --with-context --with-coroutine     --with-date_time --with-filesystem --with-iostreams --with-locale     --with-program_options --with-regex --with-serialization     --with-system --with-test --with-thread     install --prefix=/c/Boost
...
cmake -G "MinGW Makefiles" \
    ...
    -DCMAKE_EXE_LINKER_FLAGS="-static -lbcrypt" \
    ..
