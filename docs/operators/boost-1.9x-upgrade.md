...
Node binaries from this release are built against Boost 1.91 instead of the
Boost 1.83 that Ubuntu 24.04 ships.

## Do I need to replay?

**Yes.** `shared_memory.bin` written by a Boost 1.83 build cannot be reused by a
Boost 1.91 build. This was measured, not assumed: a segment created by a 1.83
binary and reopened by an otherwise identical 1.91 binary fails at the
...
1.83 build is symmetric: it will refuse the 1.91 state and want a replay too.
...
of that range. To match the official images, build Boost 1.91 with
`link=static` and `cxxflags=-std=c++14`, or use the
`vizblockchain/vizd:boost-base-1.91-noble` image as your builder:
