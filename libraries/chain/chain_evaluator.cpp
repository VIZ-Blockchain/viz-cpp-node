            ...
            try {
                ...
                if(tokens.amount>0){
                    ...
                    for (auto &b : o.beneficiaries) {
                        ...
                        _db.get_account(b.account); // validate existence
                        //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
        ...
        void escrow_transfer_evaluator::do_apply(const escrow_transfer_operation &o) {
            try {
                ...
                _db.get_account(o.to); // validate existence
                //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
                //    FC_ASSERT(!to_account.valid, "Account flagged as invalid")
                _db.get_account(o.agent); // validate existence
                //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
        ...
        void account_validator_vote_evaluator::do_apply(const account_validator_vote_operation &o) {
            ...
            const auto &validator = _db.get_validator(o.validator);

            const auto &by_account_validator_idx = _db.get_index<validator_vote_index>().indices().get<by_account_validator>();
            auto itr = by_account_validator_idx.find(boost::make_tuple(voter.id, validator.id));

            if (itr == by_account_validator_idx.end()) {
                ...
                if(_db.has_hardfork(CHAIN_HARDFORK_5)){
                    ...
                    _db.create<validator_vote_object>([&](validator_vote_object &v) {
                        v.validator = validator.id;
                        v.account = voter.id;
                    ...
                    const auto &vidx2 = _db.get_index<validator_vote_index>().indices().get<by_account_validator>();
        ...
        void request_account_recovery_evaluator::do_apply(const request_account_recovery_operation &o) {
            database &_db = db();
            _db.get_account(o.recovery_account); // validate existence
            //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
            ...
            else                                                  // Empty string recovery account defaults to top validator
                FC_ASSERT(
        ...
        void set_account_price_evaluator::do_apply(const set_account_price_operation& op) {
            ...
            //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
            //    FC_ASSERT(!account.valid, "Account flagged as invalid")
            _db.get_account(op.account_seller); // validate existence
            //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
        ...
        void set_subaccount_price_evaluator::do_apply(const set_subaccount_price_operation& op) {
            ...
            //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
            //    FC_ASSERT(!account.valid, "Account flagged as invalid")
            _db.get_account(op.subaccount_seller); // validate existence
            //if(_db.has_hardfork(CHAIN_HARDFORK_9))//can be deleted after fix in CHAIN_HARDFORK_11
