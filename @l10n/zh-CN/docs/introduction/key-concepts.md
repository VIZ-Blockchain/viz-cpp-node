...
- 错过过多区块的验证者会受到投票惩罚，惩罚持续 `validator_miss_penalty_duration` 秒。
...
| 接收方 | 来源 |
|--------|------|
| 验证者 | 区块奖励的 `inflation_validator_percent` |
...
- `inflation_validator_percent` — 验证者的区块奖励份额
- `validator_miss_penalty_percent` / `validator_miss_penalty_duration` — 错过惩罚
