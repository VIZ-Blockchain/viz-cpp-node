...
| `CHAIN_EMERGENCY_WITNESS_ACCOUNT` | `"committee"` | 紧急区块生产者账户 |
| `CHAIN_EMERGENCY_WITNESS_PUBLIC_KEY` | `VIZ75CR...` | 确定性紧急签名密钥 |
| `CHAIN_EMERGENCY_EXIT_NORMAL_BLOCKS` | 21 | 退出所需的连续真实验证者区块数 |
| `CHAIN_IRREVERSIBLE_THRESHOLD` | 75% | 退出所需的计划槽位比例 |
| `CHAIN_MAX_WITNESSES` | 21 | 最大验证者槽位数 |
...
5. 覆盖验证者计划：所有 `CHAIN_MAX_WITNESSES` 槽位 → "committee"。
...
```ini
# 仅限紧急主节点
emergency-private-key = 5Jzzz...   # CHAIN_EMERGENCY_WITNESS_ACCOUNT 私钥
```
