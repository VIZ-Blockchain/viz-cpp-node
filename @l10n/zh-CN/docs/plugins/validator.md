...
| `emergency-private-key` | — | 紧急共识的 WIF 密钥；自动将 `CHAIN_EMERGENCY_WITNESS_ACCOUNT` 添加到验证者集合 |
...
在以下条件下返回 `true`：
1. 已配置 `emergency-private-key`（`_witnesses` 中的 `CHAIN_EMERGENCY_WITNESS_ACCOUNT`）。
