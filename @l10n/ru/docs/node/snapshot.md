...
Используйте этот режим, когда `shared_memory.bin` повреждён (некорректное завершение, диск заполнен, аппаратный сбой). В DLT-режиме основной `block_log` пуст, поэтому восстановление через снимок — единственный вариант.
