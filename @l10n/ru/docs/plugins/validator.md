...
| `emergency-private-key` | — | WIF-ключ для экстренного консенсуса; автоматически добавляет `CHAIN_EMERGENCY_WITNESS_ACCOUNT` в набор валидаторов |
...
1. Настроен `emergency-private-key` (`CHAIN_EMERGENCY_WITNESS_ACCOUNT` в `_witnesses`).
