...
> ```
>
> **Координация очистки:** Оба плагина читают одни и те же ключи `history-count-blocks` и
> `history-purge-interval` из `config.ini` — разделения по плагинам нет. Значения применяются
> к обоим одновременно. Оба плагина очищают с одинаковым интервалом, а `account_history`
> дополнительно вызывает `operation_history::get_min_keep_block()` как защитную проверку,
> гарантируя, что его записи никогда не будут ссылаться на уже удалённый `operation_object`.
...
- `history-count-blocks` — сохранять историю за N блоков
- `history-purge-interval` — очищать старые записи каждые N блоков вместо каждого блока (по умолчанию: 4800 ≈ 4 часа при 3 с/блок). Уменьшает фрагментацию shared-memory и снижает contention write-lock. Каждый цикл очистки обрабатывает не более `history-purge-interval` блоков; если накопилось больше — остаток переносится на следующий цикл.
