...
3. **Authority validation**: After the chain database is open, the plugin verifies that each configured active key actually has authority on-chain. Witnesses whose accounts are not found are removed from monitoring.
4. **Initial check**: An immediate check is attempted. If the node is already in sync, the result is cached so the plugin switches to its normal periodic schedule.
...
5. **Witness iteration**: For each configured witness, the on-chain signing key is checked. If null and no restore is currently in-flight (or the previous one expired), `send_witness_update` is called.
